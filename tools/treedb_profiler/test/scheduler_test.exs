defmodule TreeDbProfiler.SchedulerTest do
  use ExUnit.Case, async: true

  alias TreeDbProfiler.{Fixtures, PortfolioState, Scheduler}

  test "runs concurrent worker loops until iteration limit" do
    root =
      Path.join(
        System.tmp_dir!(),
        "treedb-profiler-scheduler-#{System.unique_integer([:positive])}"
      )
  ...
  test "duration-controlled scheduler ignores missing iteration cap" do
    root =
      Path.join(
        System.tmp_dir!(),
        "treedb-profiler-scheduler-duration-#{System.unique_integer([:positive])}"
      )
