...
pub fn append_record<T: Serialize>(
    ...
    ensure_log(path, kind)?;
    let seq = next_seq(path, kind)?;
...
fn next_seq(path: &Path, kind: &str) -> Result<u64, StoreError> {
    Ok(replay_envelopes::<serde_json::Value>(path, kind)?
        .last()
        .map(|entry| entry.seq + 1)
        .unwrap_or(1))
}

fn replay_envelopes<T: DeserializeOwned + Serialize>(
    path: &Path,
    kind: &str,
