defmodule TreeDb.Git.Credentials do
  @moduledoc false
  ...
  defp provider, do: System.get_env("TREEDB_REMOTE_CREDENTIAL_PROVIDER") || "none"

  defp resolve_env_file(credential_id) do
    with path when is_binary(path) and path != "" <-
           System.get_env("TREEDB_REMOTE_CREDENTIALS_FILE"),
      ...
      "" ->
        {:error,
         %{
           ...
           message: "TREEDB_REMOTE_CREDENTIALS_FILE is not configured."
         }}
  ...
  defp resolve_external_command(credential_id) do
    with command when is_binary(command) and command != "" <-
           System.get_env("TREEDB_REMOTE_CREDENTIAL_COMMAND"),
