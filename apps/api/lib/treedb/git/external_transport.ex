defmodule TreeDb.Git.ExternalTransport do
  @moduledoc false

  alias TreeDb.Git.RemoteUrl
  ...
  defp enabled? do
    if System.get_env("TREEDB_GIT_EXTERNAL_TRANSPORT_ENABLED") == "true" do
      :ok
    else
  ...
  defp validate_transport(remote_url, credential) do
    cond do
      RemoteUrl.ssh?(remote_url) ->
        cond do
          System.get_env("TREEDB_GIT_SSH_ENABLED") != "true" ->
          ...
          not is_binary(System.get_env("TREEDB_GIT_SSH_KNOWN_HOSTS")) ->
            {:error,
             %{
               code: "validation_error",
               message: "TREEDB_GIT_SSH_KNOWN_HOSTS is required for SSH."
             }}
  ...
  defp credential_env(remote_url, credential) do
    ...
    cond do
      RemoteUrl.ssh?(remote_url) ->
        known_hosts = System.get_env("TREEDB_GIT_SSH_KNOWN_HOSTS")
      ...
      is_binary(credential["token"]) ->
        base ++ [{"GIT_ASKPASS", askpass_script()}, {"TREEDB_GIT_SECRET", credential["token"]}]

      is_binary(credential["password"]) ->
        base ++ [{"GIT_ASKPASS", askpass_script()}, {"TREEDB_GIT_SECRET", credential["password"]}]
  ...
  defp askpass_script do
    path =
      Path.join(System.tmp_dir!(), "treedb-git-askpass-#{System.unique_integer([:positive])}.sh")
