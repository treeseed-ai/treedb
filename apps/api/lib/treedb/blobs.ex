defmodule TreeDb.Blobs do
  ...
  def read_repo(repo_id, params, principal) do
    with {:ok, scope} <- TreeDb.Capabilities.require_capability(principal, "files:read", repo_id),
         ...
         :ok <- TreeDb.Capabilities.require_paths(scope, [path]),
  ...
  def write_workspace(workspace_id, params, principal) do
    with {:ok, ctx} <- context(workspace_id, principal, "files:write"),
         ...
         {:ok, content_hash} <- TreeDb.Store.hash_bytes_base64(content_base64),
  ...
  def max_blob_bytes, do: env_int("TREEDB_MAX_BLOB_BYTES", @default_max_blob_bytes)
  ...
  defp overlay_blob(record, path) do
    with {:ok, %{"contentBase64" => content_base64}} <-
           TreeDb.Store.read_workspace_file_content(record),
         {:ok, content_hash} <- TreeDb.Store.hash_bytes_base64(content_base64 || "") do
      {:ok,
       %{
         path: path,
         encoding: "base64",
  ...
  defp enforce_size(bytes) do
    if byte_size(bytes) <= max_blob_bytes() do
      ...
      {:error, %{code: "payload_too_large", message: "Blob exceeds TREEDB_MAX_BLOB_BYTES."}}
    end
  end
