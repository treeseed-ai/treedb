defmodule TreeDb.Graph.RefreshJobs do
  @moduledoc false
  ...
  def start(ctx, params, refresh_mode, fallback_reason, stale) do
    ...
    with {:ok, saved} <- TreeDb.Store.put_graph_refresh_job(record), do: {:ok, saved}
  end
  ...
  def complete(job, graph_version, indexed_path_count, removed_path_count) do
    ...
    TreeDb.Store.put_graph_refresh_job(record)
  end
  ...
  def get(repo_id, job_id, params, principal) do
    with {:ok, scope} <-
           TreeDb.Capabilities.require_capability(principal, "graph:query", repo_id),
         ...
         {:ok, job} <- TreeDb.Store.get_graph_refresh_job(repo_id, job_id),
