defmodule TreeDb.Uploads do
  ...
  defp workspace_context(workspace_id, principal) do
    with {:ok, workspace} when is_map(workspace) <- TreeDb.Store.get_workspace(workspace_id),
         ...
         {:ok, workspace, _scope} <-
           TreeDb.Workspaces.ensure_policy_current(workspace, principal, "files:write") do
      {:ok, workspace}
    else
  ...
  defp upload_dir(workspace_id, upload_id),
    do: Path.join([TreeDb.Store.data_dir(), "tmp", "uploads", workspace_id, upload_id])
  ...
  defp hash_bytes(bytes) do
    case TreeDb.Store.hash_bytes_base64(Base.encode64(bytes)) do
      {:ok, hash} -> hash
      _ -> "blake3:" <> (:crypto.hash(:sha256, bytes) |> Base.encode16(case: :lower))
    end
  end

  defp part_bytes, do: env_int("TREEDB_MULTIPART_PART_BYTES", 8_388_608)
  defp max_total_bytes, do: env_int("TREEDB_MAX_MULTIPART_BLOB_BYTES", 536_870_912)
  defp ttl_seconds, do: env_int("TREEDB_UPLOAD_SESSION_TTL_SECONDS", 86_400)
