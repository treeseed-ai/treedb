defmodule TreeDb.Search.Index do
  @moduledoc false

  alias TreeDb.Files.PathPolicy
  alias TreeDb.RepositoryQuery.{Document, PathMatch}

  @extensions ~w(.md .mdx .txt .json)

  def refresh(repo_id, params, principal) do
    with {:ok, ctx} <- TreeDb.RepositoryQuery.context(repo_id, params, principal, "files:search"),
  ...
  def status(repo_id, params, principal) do
    with {:ok, ctx} <- TreeDb.RepositoryQuery.context(repo_id, params, principal, "files:search"),
         {:ok, manifest} <- TreeDb.Store.get_search_index_manifest(repo_id, ctx.ref),
         {:ok, segments} <- TreeDb.Store.list_search_index_segments(repo_id, ctx.ref) do
      {:ok,
       %{
         index: %{
           repoId: repo_id,
  ...
  def compact(repo_id, params, principal) do
    with {:ok, _scope} <-
         ...
         {:ok, result} <-
           TreeDb.Store.compact_search_index(%{
             repoId: repo_id,
             refName: ref,
  ...
  defp write_segment(ctx, documents) do
    ...
    TreeDb.Store.put_search_index_segment(%{
  ...
  defp write_manifest(ctx, documents, segment, graph_manifest) do
    ...
    TreeDb.Store.put_search_index_manifest(%{
      indexVersion:
  ...
  defp latest_graph_manifest(repo_id, ref) do
    case TreeDb.Graph.Native.read_latest_graph_manifest(repo_id, ref) do
      {:ok, nil} -> {:ok, nil}
      other -> other
    end
  end
  ...
  defp hash_payload(payload) do
    {:ok, hash} = TreeDb.Store.hash_bytes_base64(payload |> Jason.encode!() |> Base.encode64())
    hash
  end
