defmodule TreeDb.Auth do
  @moduledoc false

  def mode, do: System.get_env("TREEDB_AUTH_MODE") || "dev"

  def create_dev_token(params \\ %{}) do
    if mode() != "dev" do
      {:error, %{code: "not_implemented", message: "Dev tokens are only available in dev mode."}}
    else
      actor_id = Map.get(params, "actorId") || Map.get(params, :actor_id) || "actor_demo"
      tenant_id = Map.get(params, "tenantId") || Map.get(params, :tenant_id) || "tenant_demo"
      ttl = Map.get(params, "expiresInSeconds") || Map.get(params, :expires_in_seconds) || 3600
      token = TreeDb.Ids.token()
      {:ok, token_hash} = TreeDb.Store.hash_token(token)
      ...
      TreeDb.Audit.append("auth.dev_token_created", %{actor_id: actor_id, tenant_id: tenant_id})
