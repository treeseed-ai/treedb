defmodule TreeDb.RepositoryStorage do
  @moduledoc false
  ...
  def managed_path(repository_name) do
    Path.join(TreeDb.Store.data_dir(), storage_relative_path(repository_name))
  end

  def mirror_relative_path(repository_name), do: Path.join("mirrors", repository_name)

  def mirror_path(repository_name) do
    Path.join(TreeDb.Store.data_dir(), mirror_relative_path(repository_name))
  end
  ...
  def ensure_git_repository!(path) do
    if File.exists?(Path.join(path, ".git")) or File.exists?(Path.join(path, "HEAD")) do
      ...
      case System.cmd("git", ["init", "-b", "main"], cd: path, stderr_to_stdout: true) do
        {_output, 0} ->
          File.write!(Path.join(path, ".treedbkeep"), "TreeDB managed repository\n")
          System.cmd("git", ["config", "user.email", "treedb@example.invalid"], cd: path)
          System.cmd("git", ["config", "user.name", "TreeDB"], cd: path)
          System.cmd("git", ["add", ".treedbkeep"], cd: path)
          System.cmd("git", ["commit", "-m", "Initialize TreeDB repository"], cd: path)
          :ok
