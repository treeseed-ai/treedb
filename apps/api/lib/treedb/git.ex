defmodule TreeDb.Git do
  ...
  def list_tree_recursive(path, ref_name, tree_path \\ nil),
    do: call(&TreeDb.Native.list_tree_recursive/3, [path, ref_name, tree_path])
  ...
  def commit_overlay(input) do
    ...
    input_path =
      Path.join(System.tmp_dir!(), "treedb-git-worker-#{System.unique_integer([:positive])}.json")
  ...
  defp commit_worker(input_path) do
    cond do
      ...
      File.exists?(Path.expand("../../target/release/treedb_git_worker", File.cwd!())) ->
        {:binary, Path.expand("../../target/release/treedb_git_worker", File.cwd!()),
         ["commit-overlay", input_path], []}
      ...
      true ->
        {:error,
         %{
           ...
           "message" => "treedb_git_worker is not available.",
           "details" => %{}
         }}
    end
  end
  ...
  defp repo_root do
    System.get_env("TREEDB_ROOT_DIR") || Path.expand("../..", File.cwd!())
  end
end
