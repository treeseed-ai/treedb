defmodule TreeDb.Federation.Catalog do
  @moduledoc false

  def local do
    {:ok, repos} = TreeDb.Store.list_repositories()
    {:ok, placements} = local_routes()
    {:ok, peers} = TreeDb.Store.list_federation_peers()
    {:ok, capacity} = TreeDb.Store.list_node_capacity()
    ...
    payload = %{
      ...
      "node" => %{
        ...
        "baseUrl" => TreeDb.Federation.NodeIdentity.base_url(),
        "publicKey" => TreeDb.Federation.NodeIdentity.public_key_pem(),
        "health" => "healthy"
      },
      "lineage" => %{
        "parentNodeIds" => Enum.map(TreeDb.Federation.Trust.parents(), &elem(&1, 0)),
    ...
    TreeDb.Federation.NodeIdentity.signed(payload)
  end
  ...
  def import(catalog, discovered_via \\ nil) do
    with :ok <- validate_catalog(catalog) do
      ...
      |> Enum.each(&TreeDb.Store.put_repository_advertisement/1)

      catalog
      |> Map.get("routes", [])
      |> Enum.each(&TreeDb.Store.put_federation_route/1)
  ...
  def routes do
    with {:ok, routes} <- TreeDb.Store.list_federation_routes() do
    ...
    with {:ok, repos} <- TreeDb.Store.list_repositories() do
      routes =
        Enum.flat_map(repos, fn repo ->
          case TreeDb.Store.get_repository_placement(repo["id"]) do
  ...
  defp advertisement(repo, routes) do
    ...
    %{
      "repositoryId" => repo["id"],
      "repositoryName" => repo["repositoryName"] || repo["name"],
      "ownerNodeId" => route["ownerNodeId"] || TreeDb.Federation.NodeIdentity.node_id(),
      "advertisedByNodeId" => TreeDb.Federation.NodeIdentity.node_id(),
  ...
  defp import_node(node, discovered_via) do
    existing =
      case TreeDb.Federation.Trust.get(node["nodeId"]) do
