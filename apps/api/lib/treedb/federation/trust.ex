defmodule TreeDb.Federation.Trust do
  @moduledoc false
  ...
  def bootstrap_parents! do
    ...
    |> Enum.each(fn {node_id, base_url} ->
      trust_states =
        if System.get_env("TREEDB_FEDERATION_AUTO_TRUST_PARENTS", "true") in ["false", "0"] do
          []
        else
          @default_trust
        end
  ...
  def put_peer(peer), do: TreeDb.Store.put_federation_peer(normalize(peer))
  def peers, do: TreeDb.Store.list_federation_peers()
  def get(node_id), do: TreeDb.Store.get_federation_peer(node_id)
  ...
  def parents do
    System.get_env("TREEDB_FEDERATION_PARENTS", "")
