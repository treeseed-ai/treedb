defmodule TreeDb.Federation.CatalogSync do
  @moduledoc false
  use GenServer
  ...
  def init(state) do
    if enabled?() do
      TreeDb.Federation.Trust.bootstrap_parents!()
      schedule()
    end
  ...
  defp sync do
    with {:ok, peers} <- TreeDb.Store.list_federation_peers() do
      peers
  ...
  defp pull_catalog(%{"id" => node_id, "baseUrl" => base_url})
    ...
    case TreeDb.Federation.HttpClient.get_json(
           node_id,
           base_url,
      ...
      {:ok, status, _headers, body} when status in 200..299 ->
        with {:ok, %{"ok" => true, "catalog" => catalog}} <- Jason.decode(body) do
          TreeDb.Federation.Catalog.import(catalog, node_id)
        end
  ...
  defp enabled?, do: System.get_env("TREEDB_FEDERATION_ENABLED", "true") not in ["false", "0"]

  defp interval do
    System.get_env("TREEDB_FEDERATION_CATALOG_SYNC_INTERVAL_MS", "5000")
    |> String.to_integer()
  end
end
