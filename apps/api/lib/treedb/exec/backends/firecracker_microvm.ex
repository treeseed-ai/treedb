defmodule TreeDb.Exec.Backends.FirecrackerMicrovm do
  @moduledoc false

  alias TreeDb.Exec.WorkerProtocol

  def run(command, cwd, timeout_ms, max_output_bytes, opts \\ %{}) do
    profile = System.get_env("TREEDB_EXEC_MICROVM_PROFILE") || "firecracker"
  ...
  defp worker_url do
    case System.get_env("TREEDB_EXEC_WORKER_URL") do
      url when is_binary(url) and url != "" ->
        {:ok, url}

      _ ->
        {:error,
         %{code: "sandbox_unavailable", message: "TREEDB_EXEC_WORKER_URL is not configured."}}
    end
  end

  defp post(url, body) do
    request = {String.to_charlist(url), WorkerProtocol.headers(body), ~c"application/json", body}
    timeout = env_int("TREEDB_EXEC_WORKER_TIMEOUT_MS", 30_000)
