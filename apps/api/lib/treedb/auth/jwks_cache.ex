defmodule TreeDb.Auth.JwksCache do
  @moduledoc false
  use GenServer
  ...
  def handle_call({:get_key, kid, fetcher}, _from, state) do
    ...
    ttl = env_int("TREEDB_JWKS_CACHE_TTL_SECONDS", 300)
    grace = env_int("TREEDB_JWKS_ROTATION_GRACE_SECONDS", 300)
  ...
  defp fetch_jwks do
    url = System.get_env("TREEDB_JWKS_URL")
    timeout = env_int("TREEDB_JWKS_REFRESH_TIMEOUT_MS", 2_000)
    ...
    with true <- is_binary(url) and url != "",
      ...
      false -> {:error, %{code: "auth_not_configured", message: "TREEDB_JWKS_URL is required."}}
      {:ok, _} -> malformed()
  ...
  defp malformed do
    TreeDb.Audit.append("auth.jwks_refresh_failed", %{
      status: "error",
