defmodule TreeDb.Auth.Verifiers.Hs256Dev do
  @moduledoc false
  ...
  def validate_config do
    ...
    case Enum.filter(required, fn {_name, value} -> !is_binary(value) or value == "" end) do
      [] -> :ok
      missing -> missing_config(missing)
    end
  end
  ...
  def verify(token) when is_binary(token) do
    with :ok <- validate_config(),
         ...
         :ok <- verify_common_claims(claims) do
      {:ok, claims}
    else
      [_one, _two] -> invalid()
      [_one] -> invalid()
  ...
  def info, do: %{type: "hs256_dev", issuer: issuer()}

  def verify_common_claims(claims) do
    with :ok <- verify_issuer(claims),
  ...
  def issuer, do: System.get_env("TREEDB_JWT_ISSUER")
  def audience, do: System.get_env("TREEDB_JWT_AUDIENCE")

  def invalid, do: {:error, %{code: "invalid_token", message: "Invalid bearer token."}}

  defp missing_config(missing) do
    names = Enum.map_join(missing, ", ", &elem(&1, 0))
    {:error, %{code: "auth_not_configured", message: "Missing connected auth config: #{names}."}}
  end
  ...
  defp verify_time(claims) do
    ...
    cond do
      ...
      !is_binary(claims["sub"]) or claims["sub"] == "" ->
        {:error, %{code: "invalid_token", message: "Token sub claim is required."}}
