defmodule TreeDb.Auth.Verifiers.JwksOidc do
  @moduledoc false

  alias TreeDb.Auth.Verifiers.Hs256Dev

  def validate_config do
    required = [
      {"TREEDB_JWT_ISSUER", Hs256Dev.issuer()},
      {"TREEDB_JWT_AUDIENCE", Hs256Dev.audience()},
      {"TREEDB_JWKS_URL", System.get_env("TREEDB_JWKS_URL")}
    ]
  ...
  def verify(token) when is_binary(token) do
    with :ok <- validate_config(),
         ...
         {:ok, jwk} <- TreeDb.Auth.JwksCache.get_key(kid),
  ...
  def info,
    do: %{
      ...
      jwksUrl: redact_url(System.get_env("TREEDB_JWKS_URL"))
    }
  ...
  defp allowed_algs do
    (System.get_env("TREEDB_JWT_ALLOWED_ALGS") || "RS256")
