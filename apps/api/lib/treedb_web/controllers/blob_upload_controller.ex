defmodule TreeDbWeb.BlobUploadController do
  ...
  def create(conn, %{"workspace_id" => workspace_id} = params) do
    maybe_proxy_workspace(conn, workspace_id, params, fn conn ->
      with {:ok, principal} <- require_principal(conn) do
        handle_result(conn, TreeDb.Uploads.create(workspace_id, params, principal))
      else
      ...
      case TreeDb.Federation.Proxy.maybe_proxy_workspace(workspace_id, conn, bytes) do
        :local ->
          with {:ok, principal} <- require_principal(conn),
               {:ok, payload} <-
                 TreeDb.Uploads.put_part(workspace_id, upload_id, part_number, bytes, principal) do
  ...
  def complete(conn, %{"workspace_id" => workspace_id, "upload_id" => upload_id} = params) do
    maybe_proxy_workspace(conn, workspace_id, params, fn conn ->
      with {:ok, principal} <- require_principal(conn) do
        handle_result(conn, TreeDb.Uploads.complete(workspace_id, upload_id, params, principal))
      else
        {:error, error} -> error(conn, status_for(error[:code] || error["code"]), error)
      end
    end)
  ...
  def abort(conn, %{"workspace_id" => workspace_id, "upload_id" => upload_id}) do
    maybe_proxy_workspace(conn, workspace_id, fn conn ->
      with {:ok, principal} <- require_principal(conn) do
        handle_result(conn, TreeDb.Uploads.abort(workspace_id, upload_id, principal))
      else
        {:error, error} -> error(conn, status_for(error[:code] || error["code"]), error)
      end
    end)
  ...
  defp read_limited_body(conn) do
    limit =
      System.get_env("TREEDB_MULTIPART_PART_BYTES", "8388608")
      |> Integer.parse()
      |> case do
