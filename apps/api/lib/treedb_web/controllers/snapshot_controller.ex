defmodule TreeDbWeb.SnapshotController do
  ...
  def build(conn, %{"repo_id" => repo_id} = params) do
    maybe_proxy_repo_write(conn, repo_id, params, fn conn ->
      with {:ok, principal} <- require_principal(conn) do
        handle_result(conn, TreeDb.Snapshots.build(repo_id, params, principal))
      else
  ...
  def show(conn, %{"repo_id" => repo_id, "snapshot_id" => snapshot_id}) do
    maybe_proxy_repo_read(conn, repo_id, fn conn ->
      with {:ok, principal} <- require_principal(conn) do
        handle_result(conn, TreeDb.Snapshots.get(repo_id, snapshot_id, principal))
      else
        {:error, error} -> error(conn, status_for(error[:code] || error["code"]), error)
      end
    end)
    ...
    maybe_proxy_repo_write(conn, repo_id, params, fn conn ->
      with {:ok, principal} <- require_principal(conn) do
        if download?(params) do
          case TreeDb.Snapshots.download(repo_id, params, principal) do
            {:ok, %{artifact: artifact, snapshot: snapshot, bytes: bytes}} ->
              filename = "treedb-#{repo_id}-#{snapshot["snapshotId"]}.tar.zst"

              conn
              |> put_resp_content_type("application/zstd")
              |> put_resp_header("content-disposition", "attachment; filename=\"#{filename}\"")
              |> put_resp_header("x-treedb-snapshot-id", snapshot["snapshotId"])
              |> put_resp_header("x-treedb-artifact-checksum", artifact["checksum"])
              |> send_resp(200, bytes)
          ...
          handle_result(conn, TreeDb.Snapshots.export(repo_id, params, principal))
        end
      else
        {:error, error} -> error(conn, status_for(error[:code] || error["code"]), error)
      end
