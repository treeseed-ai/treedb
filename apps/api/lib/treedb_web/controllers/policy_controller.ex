defmodule TreeDbWeb.PolicyController do
  ...
  def refresh(conn, params) do
    ...
    if TreeDb.Auth.mode() == "dev" and revocations == [] do
      ...
      with {:ok, principal} <- require_principal(conn),
        ...
        ok(conn, %{refreshed: applied != [], mode: TreeDb.Auth.mode(), refresh: record})
      else
  ...
  defp apply_revocations(revocations, principal, request_id) when is_list(revocations) do
    applied =
      Enum.map(revocations, fn revocation ->
        ...
        {:ok, grants} = TreeDb.Capabilities.list_grants(%{})
        ...
        case Enum.find(grants, &(&1["id"] == grant_id)) do
          ...
          grant ->
            ...
            {:ok, _} = TreeDb.Capabilities.put_grant(revoked)

            TreeDb.Audit.append("policy.revocation_applied", %{
              actor_id: principal["actorId"],
              tenant_id: principal["tenantId"],
              operation: "policy.refresh",
              status: "ok",
