defmodule TreeDbWeb.BlobController do
  ...
  defp read_limited_body(conn) do
    limit = TreeDb.Blobs.max_blob_bytes()
  ...
  defp upload_params(conn, params) do
    ...
    |> Map.put(
      "expectedSha",
      List.first(get_req_header(conn, "x-treedb-expected-sha")) || params["expectedSha"]
    )
    |> Map.put(
      "expectedContentHash",
      List.first(get_req_header(conn, "x-treedb-expected-content-hash")) ||
        params["expectedContentHash"]
    )
    |> Map.put(
      "allowProtected",
      List.first(get_req_header(conn, "x-treedb-allow-protected")) || params["allowProtected"]
    )
  end
