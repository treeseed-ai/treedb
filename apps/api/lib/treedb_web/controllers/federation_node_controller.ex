defmodule TreeDbWeb.FederationNodeController do
  use Phoenix.Controller, formats: [:json]
  import TreeDbWeb.ControllerHelpers

  def register(conn, params) do
    with {:ok, principal} <- require_principal(conn),
         {:ok, _scope} <- TreeDb.Capabilities.require_capability(principal, "federation:write"),
         {:ok, peer} <- TreeDb.Federation.Trust.put_peer(registration_peer(params)) do
  ...
  def index(conn, _params) do
    with {:ok, principal} <- require_principal(conn),
         ...
         {:ok, peers} <- TreeDb.Federation.Trust.peers() do
  ...
  def show(conn, %{"node_id" => node_id}) do
    with {:ok, principal} <- require_principal(conn),
         {:ok, _scope} <- TreeDb.Capabilities.require_capability(principal, "federation:read"),
         {:ok, peer} when is_map(peer) <- TreeDb.Federation.Trust.get(node_id) do
      ok(conn, %{peer: sanitize(peer)})
    else
  ...
  def trust(conn, %{"node_id" => node_id} = params) do
    ...
    with {:ok, principal} <- require_principal(conn),
         {:ok, _scope} <- TreeDb.Capabilities.require_capability(principal, "federation:trust"),
         {:ok, peer} <- TreeDb.Federation.Trust.trust(node_id, states) do
      ok(conn, %{peer: sanitize(peer)})
    else
      {:error, error} -> error(conn, status_for(error[:code] || error["code"]), error)
    end
  ...
  def revoke(conn, %{"node_id" => node_id}) do
    with {:ok, principal} <- require_principal(conn),
         {:ok, _scope} <- TreeDb.Capabilities.require_capability(principal, "federation:trust"),
         {:ok, peer} <- TreeDb.Federation.Trust.revoke(node_id) do
      ok(conn, %{peer: sanitize(peer)})
    else
      {:error, error} -> error(conn, status_for(error[:code] || error["code"]), error)
    end
