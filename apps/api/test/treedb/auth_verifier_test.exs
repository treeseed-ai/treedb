defmodule TreeDb.AuthVerifierTest do
  ...
  setup do
    ...
    dir =
      Path.join(
        System.tmp_dir!(),
        "treedb-auth-verifier-test-#{System.unique_integer([:positive])}"
      )
  ...
  test "connected mode refuses missing verifier config" do
    ...
    System.delete_env("TREEDB_AUTH_VERIFIER")
    System.delete_env("TREEDB_JWT_HS256_SECRET")

    assert {:error, %{code: "auth_not_configured"}} = TreeDb.Auth.validate_boot_config()
  end
  ...
  test "jwks_oidc verifies RS256 tokens, refreshes rotated keys, and rejects unsupported algs" do
    ...
    assert {:error, %{code: "invalid_token"}} = TreeDb.Auth.authenticate_token(unsigned)
  end
  ...
  test "jwks_oidc rejects missing exp, wrong issuer, wrong audience, and malformed jwks" do
    ...
    assert {:error, %{code: "invalid_token"}} =
             TreeDb.Auth.authenticate_token(
    ...
    assert {:error, %{code: "invalid_token"}} =
             TreeDb.Auth.authenticate_token(rs256_jwt(private_key, "kid-a", valid_claims()))
  end

  defp configure_hs256! do
    System.put_env("TREEDB_AUTH_MODE", "connected")
    System.put_env("TREEDB_AUTH_VERIFIER", "hs256_dev")
  ...
  defp configure_jwks!(path) do
    System.put_env("TREEDB_AUTH_MODE", "connected")
    System.put_env("TREEDB_AUTH_VERIFIER", "jwks_oidc")
    System.put_env("TREEDB_JWT_ISSUER", "https://issuer.example.invalid")
    System.put_env("TREEDB_JWT_AUDIENCE", "treedb")
    System.put_env("TREEDB_JWKS_URL", "file://#{path}")
    System.put_env("TREEDB_JWT_ALLOWED_ALGS", "RS256")
  end
  ...
  defp env_names do
    ~w[
      TREEDB_AUTH_MODE
      TREEDB_AUTH_VERIFIER
      TREEDB_JWT_ISSUER
      TREEDB_JWT_AUDIENCE
      TREEDB_JWT_HS256_SECRET
      TREEDB_JWKS_URL
      TREEDB_JWT_ALLOWED_ALGS
    ]
  end
end
