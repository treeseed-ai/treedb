defmodule TreeDbWeb.GraphIncrementalRefreshTest do
  ...
  setup %{conn: conn} do
    ...
    repo_path = Path.join(TreeDb.Store.data_dir(), "repos/bare/graph-incremental")
  ...
  test "records incremental graph refresh job metadata and exposes status", %{
    ...
    refute inspect(status) =~ TreeDb.Store.data_dir()
  end
