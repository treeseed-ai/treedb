defmodule TreeDbWeb.FileControllerTest do
  ...
  setup %{conn: conn} do
    ...
    path = Path.join(TreeDb.Store.data_dir(), "repos/bare/file-api")
  ...
  test "lists, reads, writes, patches, deletes, searches, diffs, and commits files", %{
    ...
    conn =
      ...
      |> put("/api/v1/workspaces/#{workspace_id}/files?path=docs/new.md", %{
        "encoding" => "utf8",
        "content" => "TreeDB overlay content"
      })
    ...
    assert json_response(conn, 200)["content"] == "TreeDB overlay content"
  end
  ...
  test "path traversal, protected paths, and missing token are rejected", %{
    ...
    previous = System.get_env("TREEDB_MAX_FILE_BYTES")
    System.put_env("TREEDB_MAX_FILE_BYTES", "4")
    ...
    after
      if previous,
        do: System.put_env("TREEDB_MAX_FILE_BYTES", previous),
        else: System.delete_env("TREEDB_MAX_FILE_BYTES")
    end
  end
