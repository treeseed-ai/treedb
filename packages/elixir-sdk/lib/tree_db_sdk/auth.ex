defmodule TreeDbSdk.AuthProvider do
  @moduledoc false
  @callback get_token(term()) :: {:ok, String.t()} | {:error, term()}
end

defmodule TreeDbSdk.Auth do
  @moduledoc false

  def static_bearer_token_provider(token), do: {:static_bearer_token, token}

  def resolve_authorization_header(%TreeDbSdk.Config{auth_provider: provider})
  ...
  def resolve_authorization_header(%TreeDbSdk.Config{token: token}) when is_binary(token) do
    {:ok, {"Authorization", "Bearer #{token}"}}
  end

  def resolve_authorization_header(%TreeDbSdk.Config{}), do: {:ok, nil}
