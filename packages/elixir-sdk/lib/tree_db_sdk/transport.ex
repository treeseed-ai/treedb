defmodule TreeDbSdk.Transport.Request do
  @moduledoc false
  defstruct [:method, :path, query: %{}, headers: %{}, body: nil, binary_body: nil]
end

defmodule TreeDbSdk.Transport.Response do
  @moduledoc false
  defstruct [:status, headers: %{}, data: nil]
end

defmodule TreeDbSdk.Transport do
  @moduledoc false
  @callback request(TreeDbSdk.Config.t(), TreeDbSdk.Transport.Request.t()) ::
              {:ok, TreeDbSdk.Transport.Response.t()} | {:error, TreeDbSdk.Error.t()}
end

defmodule TreeDbSdk.Transport.Httpc do
  @moduledoc false
  @behaviour TreeDbSdk.Transport
  ...
  def request(config, request) do
    with {:ok, url} <- build_url(config, request),
      ...
      case :httpc.request(method, http_request, options, body_format: :binary) do
        ...
        {:error, reason} ->
          {:error, TreeDbSdk.Error.network(inspect(reason))}
      end
    end
  end

  defp build_url(%TreeDbSdk.Config{base_url: base_url}, request) when is_binary(base_url) do
  ...
  defp build_url(_, _), do: {:error, TreeDbSdk.Error.network("TreeDB base_url is required")}

  defp build_headers(config, request) do
    headers = Map.merge(config.default_headers || %{}, request.headers || %{})

    with {:ok, auth_header} <- TreeDbSdk.Auth.resolve_authorization_header(config) do
      headers =
        if auth_header,
  ...
  defp build_body(%{binary_body: body}) when not is_nil(body),
    do: {:ok, TreeDbSdk.Binary.to_binary(body), ~c"application/octet-stream"}
  ...
  defp decode_response(status, headers, body) do
    ...
    if status in 200..299 do
      {:ok, %TreeDbSdk.Transport.Response{status: status, headers: header_map, data: data}}
    else
      {:error, TreeDbSdk.Error.from_response(status, data)}
    end
  end
