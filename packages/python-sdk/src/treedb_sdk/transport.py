...
T = TypeVar("T")
TreeDbHttpMethod = Literal["GET", "POST", "PUT", "PATCH", "DELETE"]
...
@dataclass(frozen=True)
class TreeDbResponse(Generic[T]):
    status: int
...
class HttpxTransport:
    ...
    def request(self, request: TreeDbRequest) -> TreeDbResponse[Any]:
        ...
        try:
            ...
            raise TreeDbApiError.network("TreeDB network request failed", error) from error

        data = _parse_response_body(response)
        if response.status_code < 200 or response.status_code >= 300:
            raise TreeDbApiError.from_response(response.status_code, data)
        return TreeDbResponse(status=response.status_code, headers=dict(response.headers), data=data)
