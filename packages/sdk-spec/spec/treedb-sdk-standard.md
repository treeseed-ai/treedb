...
scaffolding for TreeDB SDKs across TypeScript, Python, Rust, and Elixir.
...
generic TreeDB SDK and may keep downstream compatibility tests, but TreeSeed
model names, content registry semantics, UI/site workflow behavior, and product
compatibility expectations must not define the generic TreeDB SDK architecture.

The generic SDKs stay TreeDB-only: repositories, refs, paths, workspaces, graph,
...
The core layer owns shared TreeDB behavior. The facade layer exposes idiomatic
...
and adapters for TreeDB modules. Core module ownership is machine-readable in
...
| --- | --- | --- |
| TypeScript | `packages/ts-sdk` | `@treedb/ts-sdk` |
| Python | `packages/python-sdk` | `treedb-sdk` / `treedb_sdk` |
| Rust | `packages/rust-sdk` | `treedb-sdk` / `treedb_sdk` |
| Elixir | `packages/elixir-sdk` | `:tree_db_sdk` / `TreeDbSdk` |
...
TreeDB response envelopes, binary bodies, and error metadata.

## Error Contract

SDKs expose `TreeDbApiError`-compatible failures with status, code, message,
details, and payload.

| Field | Meaning |
| --- | --- |
| status | HTTP status code, or 0 for network failures |
| code | Stable TreeDB error code from OpenAPI |
...
| Concept | Requirement |
| --- | --- |
| TreeDbCursor | Opaque server-owned string |
| TreeDbPage.items | Preserved result items |
| TreeDbPage.nextCursor | Exposed without decoding |
| TreeDbPage.hasMore | Preserved when returned |
...
Exec APIs cover workspace-scoped command execution under TreeDB policy
boundaries.
...
| integration | yes | yes | Real TreeDB server tests |
...
TreeDB language SDK architecture or replace shared conformance.
