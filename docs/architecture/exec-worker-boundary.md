...
TreeDB supports four exec backend names:
...
TreeDB sends a reduced workspace execution request containing command, mode,
resource limits, network policy, and authorized path scope. TreeDB does not
embed or operate Firecracker directly; the microVM profile is an operator-managed
worker implementation detail.

Worker requests may be signed with `TREEDB_EXEC_WORKER_HMAC_SECRET`. Audit
