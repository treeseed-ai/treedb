...
- A TypeScript TreeDB SDK implementation aligned with the same shared architecture as every other language SDK.
...
- `packages/trsd-sdk` is the existing `@treeseed/sdk` package and is useful as an integration reference for connected authentication, permissions, and higher-level SDK consumption of TreeDB.
- `packages/trsd-sdk` is not the TreeDB TypeScript SDK and must not define the TreeDB SDK architecture.
- The future `packages/ts-sdk` package is the TypeScript TreeDB SDK package. It is a new TreeDB SDK project, not a copy, rename, symlink, or alias of `packages/trsd-sdk`.
...
packages/trsd-sdk
  ...
  reference consumer for future TreeDB SDK integration
  not the canonical TreeDB TypeScript SDK
  currently has no tracked src/treedb implementation in this checkout
  existing tests include workflow suites with side effects in temporary repos

packages/ts-sdk
  future TypeScript TreeDB SDK package
...
docs/research/sdk-integration-design.md
  existing AgentSdk / TreeDB design notes
...
```text
The TreeDB TypeScript SDK package is packages/ts-sdk.

packages/trsd-sdk is not the TreeDB TypeScript SDK. It is the existing
@treeseed/sdk package and is useful as an integration reference because it
shows how a higher-level product SDK may consume TreeDB capabilities, connected
authentication, and permissions. The TreeDB SDK packages must remain generic
...
Create packages/ts-sdk as the TypeScript TreeDB SDK package.
...
```text
mix test test/treedb_web/openapi_contract_test.exs
  pass: 3 tests, 0 failures

mix test test/treedb_web/route_openapi_inventory_test.exs
  pass: 1 test, 0 failures

./scripts/openapi-check.sh
  initial observed result: blocked because the TreeDB data directory was already locked by PID 1223583
...
- `packages/trsd-sdk` will consume `packages/ts-sdk` later instead of directly embedding TreeDB database calls.
- TreeDB SDK APIs must stay generic and OpenAPI-aligned: repo, ref, path, workspace, file/blob, graph, search, context, auth, policy, and capability.
...
```text
The TreeDB TypeScript SDK package is packages/ts-sdk.
packages/ts-sdk is a future generic TreeDB SDK package, not a copy or alias of
...
`treedb-sdk-standard.md` documents the same architecture for humans; and
...
Downstream product SDKs such as `packages/trsd-sdk` may have extra compatibility tests for TreeSeed migration safety, but TreeDB language SDK tests must follow the same required structure across TypeScript, Python, Rust, and Elixir.
...
required_test_categories:
  ...
  compatibility:
    description: Migration-specific tests for downstream product SDKs that consume TreeDB SDKs.
...
rules:
  ...
  - Compatibility tests must not define the cross-language TreeDB SDK architecture.
...
TreeDB language SDKs do not require compatibility roots.
Downstream product SDK manifests may declare compatibility roots when they consume TreeDB SDKs.
...
- Compatibility is documented as downstream product SDK migration-only, not as a TreeDB language SDK architecture category.
...
`TreeDbApiError`-compatible errors, bearer auth providers, opaque cursor
...
## Phase 8 — Create the TypeScript TreeDB SDK

### Goal

Create `packages/ts-sdk` as the generic TypeScript TreeDB SDK. It must implement the same shared SDK architecture as Python, Rust, and Elixir and must not depend on TreeSeed product concepts.
...
Phase 8 is complete. `packages/ts-sdk` exists as the generic TypeScript TreeDB
...
## Phase 9 — Integrate TreeSeed SDK Through the TypeScript TreeDB SDK

### Goal

Migrate `packages/trsd-sdk` TreeDB-related behavior to consume standardized `packages/ts-sdk` clients and adapters without breaking the TreeSeed SDK public surface.

### Repository Changes

In `packages/trsd-sdk`, introduce backend interfaces that adapt TreeSeed concepts to generic TreeDB SDK calls:
...
```text
LocalContentBackend
TreeDbContentBackend using packages/ts-sdk
```
...
GraphBackend
LocalGraphBackend
TreeDbGraphBackend using packages/ts-sdk
```
...
ExecBackend
LocalExecBackend
TreeDbExecBackend using packages/ts-sdk
```
...
```text
Before: direct TreeDB-specific helper or local ContentStore path in packages/trsd-sdk.
After: selected ContentBackend using standardized packages/ts-sdk TreeDbClient adapters.
```
...
- TreeDB-backed operations use standardized `packages/ts-sdk` `TreeDbClient` and adapters.
...
Phase 9 is complete. `packages/trsd-sdk` uses `packages/ts-sdk` as its TreeDB
access layer. TreeDB is the default adapter for the TreeSeed project content
repository as a portfolio of repositories, not a single configured repository.
`AgentSdk` config supplies TreeDB service/auth/ref/workspace context and
optional repository hints, while repository IDs are discovered internally only
when repo-scoped TreeDB endpoints require them. Local filesystem/git remains the
...
Phase 10 is complete. `packages/python-sdk` exists as the generic Python TreeDB
...
packages/rust-sdk/
  ...
  sdk-manifest.yaml
  scripts/
    generate_treedb_openapi_types.mjs
    check_treedb_generated_types.mjs
  src/
    lib.rs
    auth.rs
    binary.rs
...
Phase 11 is complete. `packages/rust-sdk` exists as the generic Rust TreeDB SDK
...
packages/elixir-sdk/
  ...
  sdk-manifest.yaml
  scripts/
    generate_treedb_openapi_types.exs
    check_treedb_generated_types.exs
  lib/
    tree_db_sdk.ex
    tree_db_sdk/
      auth.ex
...
Phase 12 is complete. `packages/elixir-sdk` exists as the generic Elixir TreeDB
...
`SDK Spec` validates the shared spec package:
...
`SDK Packages` validates all four language SDK package baselines:

```text
TypeScript: generated metadata check, build, unit/adapter/generated/integration tests, full npm test
Python: generated metadata check, editable dev install, build, unit/adapter/generated/integration tests, full pytest
Rust: generated metadata check, cargo fmt --check, cargo clippy -D warnings, cargo test
Elixir: generated metadata check, mix format --check-formatted, mix test
```

`SDK Conformance` validates shared scenario catalog loading and clean
`not_configured` behavior in every SDK conformance adapter. It does not start a
TreeDB server in Phase 13 because executable live conformance dispatch remains
later work.

`SDK Integration` is optional/manual and secret-aware. It runs each language's
integration tests with:

```text
TREEDB_BASE_URL: workflow input base_url or TREEDB_SDK_BASE_URL secret
TREEDB_TOKEN: TREEDB_SDK_TOKEN secret
```

If `TREEDB_BASE_URL` is absent, package integration tests must pass cleanly by
reporting or skipping not-configured behavior.
...
If local Python `pip` is available, also run:
...
- `SDK Spec` workflow exists and validates sdk-spec, OpenAPI declared coverage,
  manifests, matrix rendering, and sdk-spec tests.
- `SDK Packages` workflow exists and validates TypeScript, Python, Rust, and
  Elixir package checks.
- `SDK Conformance` workflow exists and validates scenario loading plus clean
  `not_configured` conformance adapter behavior.
- `SDK Integration` workflow exists and runs optional live/not-configured
  integration checks without requiring live credentials.
- `packages/sdk-spec/README.md` documents workflow scope and local equivalents.
- Root `TreeDB Release Gate` remains separate.

Phase 13 is complete. GitHub Actions workflows now validate `sdk-spec`, all four
language SDK packages, shared conformance scenario loading, and optional live SDK
integration checks. The SDK workflows are separate from the root TreeDB release
gate so SDK-affecting changes can require SDK checks without forcing the service
release gate to own every language toolchain by default. Current conformance
workflows validate scenario catalog loading and clean `not_configured` behavior;
live executable conformance remains later work.
...
Root `scripts/release-gate.sh` remains focused on the TreeDB service, native
...
3. Root OpenAPI gate
4. Optional `SDK Integration` when configured

`SDK Integration` remains optional/manual and secret-backed in Phase 14.
Current SDK conformance remains scenario catalog loading plus clean
`not_configured` behavior; executable live conformance remains later work.
...
SDK package verification. `docs/runbooks/sdk-release.md` documents local SDK
verification, required SDK workflows, optional live integration behavior,
focused TreeSeed regression, release candidate readiness, troubleshooting, and
cleanup.
...
```text
TreeDB is the default adapter for project content when configured
...
No global TreeDB repository id
```
...
TreeDB repository-id wording, required README topics, required command
...
- SDK docs do not reintroduce global TreeDB repository-id configuration.
...
now describe portfolio-backed TreeDB content without a global repository id.
`scripts/check-sdk-docs.sh` verifies required documentation files, key command
references, stale TreeDB repo-id language, and Phase 15 completion status.
...
The full root `scripts/release-gate.sh` remains required for complete TreeDB
...
Phase 16 is complete. The TreeDB SDK baseline now includes `sdk-spec`, generic
...
packages/ts-sdk
  provides an idiomatic TypeScript TreeDB SDK with the same architecture as the other language SDKs

packages/trsd-sdk
  remains the existing TreeSeed SDK and consumes packages/ts-sdk for TreeDB access
